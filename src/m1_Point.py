...
class Point():
    ...
    def move_to(self, x, y):
        ...
        self.y = y
        self.moves = self.moves +1
        distance = math.sqrt((self.x - initial_x)**2 + (self.y - initial_y)**2)
        self.distance = self.distance + distance

    def get_number_of_moves_made(self):
        return self.moves

    def get_distance_from(self, n):
    ...
    def closer_to(self, n, m):
        distance_n = self.get_distance_from(n)
        distance_m = self.get_distance_from(m)
        if distance_n <= distance_m:
            return n
        else:
            return m

    def halfway_to(self,n):
        halfway_x = (self.x + n.x)/2
        halfway_y = (self.y + n.y)/2
